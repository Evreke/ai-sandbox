...
/**
 * Open the interactive deck TUI for the given questions.
 * <p>
 * FUNCTION_CONTRACT:
 * Input:
 *   - ctx: needs ctx.ui (pi custom-component surface) and TUI mode
 *   - questions: sanitized deck questions (rendered verbatim)
 *   - prefill: optional answers to pre-apply (revision decks from /grill)
 * Output: Promise<DeckOutcome | null> — answers map when submitted,
 *   {cancelled:true} on Esc, null when not in TUI mode
 * Guarantees:
 *   - submit only resolves once EVERY question is answered or deferred
 *   - fresh decks (hadOpenAtStart) auto-submit when the last question settles;
 *     revision decks keep ctrl+s manual so several answers can change first
 *   - every free-typed answer label passes through clean() (SECURITY-AUDIT F1)
 *   - all rendered lines are wrapped to the terminal width (narrow-terminal
 *     crash fix, see BUG_FIX_CONTEXT in render)
 * Raises: never throws past the caller
 */
function openDeck(
...
/**
 * grill-deck extension entry point: registers the `grill_deck` tool and the
 * `/grill` revision command.
 * <p>
 * FUNCTION_CONTRACT (default export):
 * Input:
 *   - pi: the extension API pi passes on load
 * Output: none — registers:
 *   - tool grill_deck: sanitizes + caps the question frontier (MAX_QUESTIONS),
 *     opens the deck, persists the round as session entry
 *     "grill-deck-round", returns answersToText(round, …)
 *   - command /grill: reopens the last round pre-filled; a changed answer set
 *     is persisted as "grill-deck-revision" and re-sent via sendUserMessage
 * Guarantees:
 *   - non-TUI calls degrade to a markdown-list instruction, never throw
 *   - session state is rebuilt defensively on session_start (lib.ts parsers
 *     drop malformed entries individually — SECURITY-AUDIT F2)
 *   - model-supplied strings are sanitized (clean) before render or persist
 * Raises: none past the handlers
 */
export default function grillDeck(pi: ExtensionAPI) {
	function replayFromSession(ctx: ExtensionContext) {
		// EXTERNAL_DEPENDENCY: pi's session store — the branch entries this
		// session already persisted (custom entries "grill-deck-round" /
		// "grill-deck-revision"); replay is the only state restore, there is
		// no file/socket dependency of its own.
		rounds.length = 0;
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			rounds.push(record);
			// EXTERNAL_DEPENDENCY: pi's session persistence (appendEntry writes a
			// custom entry into the session JSONL) — the deck's only durable state.
